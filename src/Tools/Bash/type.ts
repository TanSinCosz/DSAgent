...
For commands that are harder to parse at a glance (long flags, package-script arguments, or scoped git inspection), add enough context to clarify what it does:
- git diff -- src/Tools/Bash/prompt.ts -> "Show Bash prompt changes"
- npm run test:auto-compress -> "Run auto-compress tests"
- node --test tests/agent-tool.test.ts -> "Run Agent tool tests"`),
